    ...
    Plugin = function(options) {
        ...
        $.extend(Plugin.prototype, {
            init: function(selector) {
                ...
                if (this.settings.onCreatedCallback) {
                    this.settings.onCreatedCallback({
                        instance: this.dom.slideme_container,
                        index: this.counters.current
                    });
                }
            ...
            animate: function(data) {
                if (this.counters.current !== this.counters.next) {
                    ...
                    if (this.settings.onStartCallback) {
                        this.settings.onStartCallback({
                            instance: this.dom.slideme_container,
                            index: this.counters.current,
                            total: this.counters.total,
                            next: this.counters.next
                        });
                    }
                    if (data.direction === 1) {
                    ...
                    if (this.settings.css3) {
                        ...
                        var currentAnimation = {}, nextAnimation = {};
            ...
            createArrows: function(event) {
                var arrows = {};
                for (var key in this.settings.labels) {
                    var params = {
                        direction: key === "next" ? 1 : -1,
                        key: key
                    };
            ...
            createPagination: function(event) {
                ...
                if (this.settings.pagination === "numbers" || this.settings.pagination === "both") {
                    ...
                    event = this.settings.nativeTouchScroll ? this.beforeScrollTo : this.playTo;
            ...
            createThumbs: function(event) {
                var thumbs = [], width = this.settings.thumbs.width, height = this.settings.thumbs.height;
                this.dom.slideme.find(">*").each(function(i) {
                    var params = {
                        index: i
                    }, src = $(this).find("img").eq(0).attr("src"), img = $("<img />").attr({
                        src: src,
                        width: width,
                        height: height
            ...
            onResize: function() {
                var width = this.settings.resizable.width, height = this.settings.resizable.height, h = Math.round(height * this.dom.slideme.width() / width);
                this.dom.slideme.css({
                    height: h
                });
            },
            ...
            setInfiniteSlides: function() {
                ...
                if (this.counters.total !== 1) {
                    if (!this.dom.current.next().length) {
                        slide = this.dom.slideme.children(":first-child").detach();
                        this.counters.current = this.counters.current - 1;
                        this.dom.slideme.append(slide);
                    } else if (!this.dom.current.prev().length) {
                        slide = this.dom.slideme.children(":last-child").detach();
                        this.counters.current = this.counters.current + 1;
            ...
            swipeSupport: function() {
                $("body").prepend("<div>" + document.documentElement.ontouchstart + " : " + window.ontouchstart + "</div>");
