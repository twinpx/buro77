( function($) {
  ...
  $( function() {
      ...
      timerId = setTimeout( function() {
        ...
        $.ajax({
          ...
          beforeSend: function() {
            ...
            Cookies.set( 'filter'+elementId, $form.serialize(), { expires: 365, path: window.location.pathname });
          },
    ...
    //set page query from cookie if there is no query yet
    ...
    for ( var key in query ) {
      ...
      if ( $elem.length ) {
        switch( $elem.attr( 'type' )) {
          case 'radio':
            if ( query[ key ]) {
              $elem.closest( '.b-filter__menu-item, .b-filter__sort-item' ).find( '[value=' + query[ key ] + ']' ).attr({ checked: 'checked' }).change();
            }
          break;
          case 'hidden':
            $elem.val( query[ key ]);
            ...
            if ( key === 'MIN_STAGE' && query.MAX_STAGE) {
              slideSlide( null, { values: [query.MIN_STAGE, query.MAX_STAGE]} );
              $( "#stageRangeSlider" ).slider( "values", 0, query.MIN_STAGE );
              $( "#stageRangeSlider" ).slider( "values", 1, query.MAX_STAGE );
              slideChange( null, { values: [query.MIN_STAGE, query.MAX_STAGE]} );
            }
